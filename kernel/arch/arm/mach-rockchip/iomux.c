...
MUX_CFG(GPIOE_SPI1_FLASH_SEL2_NAME,		A,   20,    2,	  0,	DEFAULT)		/* 00 : gpio_e1/e2/e3/f7 01 : spi1_clkin/spi1_ss_in_n/spi1_rxd/spi1_txd  10 :flash_cs6 / flash_cs7*/
...
MUX_CFG(GPIOD_LCDC16BIT_SEL_NAME,			B,   10,    1,	  1,	INITIAL)		/* 0 : gpio_d0 ~ gpio_d7 1 : lcdc_data8 ~ lcdc_data15 */
...
MUX_CFG(GPIOA4_I2SLRCK_SEL_NAME,			B,    5,    1,	  0,	DEFAULT)		/* 0 : gpio0_a4 1 : i2s lrck_rx */
MUX_CFG(CXGPIO_HOST_SEL_NAME,				B,    4,    1,	  0,	DEFAULT)		/* 0 : gpio2_0 ~ gpio2_13 1 : host interface */
MUX_CFG(GPIOH7_VIPDATA3_SEL_NAME,			B,    3,    1,	  0,	DEFAULT)		/* 0 : gpio_h7 1 : vip data 3 */
MUX_CFG(GPIOH6_VIPDATA2_SEL_NAME,			B,    2,    1,	  0,	DEFAULT)		/* 0 : gpio_h6 1 : vip data2 */
...
void rockchip_mux_set(struct mux_config *cfg)
{
	...
	#ifdef DEBUG_LHH
	printk("%s::regValue is %x,mask is %x,reg=0x%08x\n",
	        __FUNCTION__,regValue,mask,cfg->mux_reg);
	//dump_stack();
	#endif
