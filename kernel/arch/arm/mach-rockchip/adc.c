...
int32 ADCStart(uint8 ch)
{
    ...
    pheadAdc->ADC_CTRL = (pheadAdc->ADC_CTRL & (~0x1f)); // HSL@RK,power off first.
    if(ch>3)
    {
        return (-1);
    }
...
static void get_rock_adc_wait_ready() 
{
	int count = 0;
	...
	//	printk("get_rock_adc_sync count = %d \n", count);
	//	if (count >= 100) {
	//		printk("error get_rock_adc_sync \n");
	//	} 
}

void get_rock_adc_scanning_end()
{
	get_rock_adc_wait_ready();
}

void get_rock_adc_scanning_restart()
{
	if (g_adcch>=Adc_channel_max)
		g_adcch = Adc_channel0;
