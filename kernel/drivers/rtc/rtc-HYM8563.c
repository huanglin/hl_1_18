...
static ssize_t rtc_attr_show(struct device *dev,struct device_attribute *attr, char *buf)
{
	...
	printk("\n->rtc_attr_show-->r_min=%d r_hour=%d r_mday=%d r_wday=%d!\n",BCD2BIN(regs[0]& 0x7f),BCD2BIN(regs[1]& 0x3f),BCD2BIN(regs[2]& 0x3f),BCD2BIN(regs[3]& 0x7));
	regs[0]=0x0;
	hym8563_i2c_read_regs(client, RTC_CTL2, regs, 1);
	printk("%s->%d--RTC_CTL2 data= %x\n",__FUNCTION__,__LINE__,regs[0]);
...
static int hym8563_i2c_read_regs(struct i2c_client *client, u8 reg, u8 buf[], unsigned len)
{
	...
	if(ret < 0){
		printk("!!!!!!!hym8563_i2c_read_regs err, reg=0x%02x, ret=%d\n", reg, ret);
	}	
	if (ret > 0)
		ret = 0;
...
static int hym8563_i2c_set_regs(struct i2c_client *client, u8 reg, u8 const buf[], __u16 len)
{
	...
	ret = i2c_transfer(client->adapter, msgs, 1);
	if(ret < 0){
		printk("!!!!!!!hym8563_i2c_set_regs err, reg=0x%02x, val=0x%02x, ret=%d\n", reg, buf[0], ret);
	}
	if (ret > 0)
		ret = 0;
...
static int hym8563_i2c_read_time(struct i2c_client *client, struct rtc_time *tm)
{
    ...
    for(i=0;i<HYM8563_RTC_SECTION_LEN;i++){
		ret = hym8563_i2c_read_regs(client, RTC_SEC+i, &regs[i], 1);
		if(ret < 0){
			printk("hym8563_i2c_read_regs err, addr=0x%02x, val=0x%02x\n", RTC_SEC+i, regs[i]);
		}
	}
...
static int hym8563_i2c_read_alarm(struct i2c_client *client, struct rtc_wkalrm *tm)
{
	...
	DBG("\n---hym8563_i2c_read_alarm-->rmin=%d rhour=%d rmday=%d rwday=%d\n",BCD2BIN(regs[0]& 0x7f),BCD2BIN(regs[1]& 0x3f),BCD2BIN(regs[2]& 0x3f),BCD2BIN(regs[3]& 0x7));
	regs[0] = 0x0;
	hym8563_i2c_set_regs(client, RTC_CTL2, regs, 1);
	DBG(">>%s->%d---read alarm and clean disable alarm  \n",__FUNCTION__,__LINE__);
	return 0;
}
...
{	
	...
	hym8563_i2c_read_regs(client, RTC_A_MIN, regs, 4);
	//pr_info(">>%s->%d---tm->enabled=%d  tm->pending=%d\n",__FUNCTION__,__LINE__,tm->enabled,tm->pending);
	//pr_info("\ncurent alarm register->rmin=%d rhour=%d rmday=%d rwday=%d!\n",BCD2BIN(regs[0]& 0x7f),BCD2BIN(regs[1]& 0x3f),BCD2BIN(regs[2]& 0x3f),BCD2BIN(regs[3]& 0x7));
...
{
	DBG("\n**************8>>%s->%d---tm->enabled=%d  tm->pending=%d",__FUNCTION__,__LINE__,tm->enabled,tm->pending);
...
{
	DBG("\n**************>>%s->%d---tm->enabled=%d  tm->pending=%d",__FUNCTION__,__LINE__,tm->enabled,tm->pending);
...
extern void rk28_send_rtc_wakeup_key( void ) ;
static irqreturn_t rk28_rtc_irq_handler(s32 irq, void *dev_id)
{
...
static int hym8563_probe(struct i2c_adapter *adapter, int addr, int kind)
{
	...
	error = request_gpio_irq(RTC_INT_IOPIN,(pFunc)rk28_rtc_irq_handler,GPIOEdgelFalling,client);
	if(error) {
