...
static int rk1000_shutdown(struct i2c_client *client)
{
	 rockchip_mux_api_set(GPIOA23_UART2_SEL_NAME,IOMUXB_GPIO0_A23);
...
static int rk1000_control_probe(struct i2c_adapter *adapter, int addr, int kind)
{
	...
	struct i2c_client *client = NULL;
			...
			;
#else
	GPIOSetPinDirection(RK1000_RST_IOPIN,GPIO_OUT);				/*xxm*/
	GPIOSetPinLevel(RK1000_RST_IOPIN,GPIO_HIGH);
	mdelay(5);
#endif
#if 0
#if defined (CONFIG_RK2818_A7)
	GPIOSetPinDirection(GPIOPortA_Pin2,GPIO_OUT);
	GPIOPullUpDown(GPIOPortA_Pin2,GPIOPullUp);
	GPIOSetPinLevel(GPIOPortA_Pin2,GPIO_HIGH);
	mdelay(1);
