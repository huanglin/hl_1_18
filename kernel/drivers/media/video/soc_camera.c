                             ...
                             struct v4l2_ext_controls *ctrl)
{
    ...
    if (icd->ops->get_ext_control) {
        for (i=0; i<ctrl->count; i++) {        
            if (icd->ops->get_ext_control(icd, &ctrl->controls[i]) != 0) {
                error_cnt++;
                error_idx = i;
            }
        }
                             ...
                             struct v4l2_ext_controls *ctrl)
{
    ...
    if (icd->ops->set_ext_control) {
        for (i=0; i<ctrl->count; i++) {        
            if (icd->ops->set_ext_control(icd, &ctrl->controls[i]) != 0) {
                error_cnt++;
                error_idx = i;
            }
        }
