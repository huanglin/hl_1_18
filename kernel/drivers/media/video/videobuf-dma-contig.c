		...
		for (i = 0; i < VIDEO_MAX_FRAME; i++) {
			...
			if (mem) {
				...
				/*dma_free_coherent(q->dev, mem->size,
						  mem->vaddr, mem->dma_handle);*/	//nzy delete
				mem->vaddr = NULL;
			}
				  ...
				  struct vm_area_struct *vma)
{
	...
	MAGIC_CHECK(mem->magic, MAGIC_DC_MEM);
        ...
        mem->vaddr = dma_alloc_coherent(NULL, mem->size,//nzy modify
                        &mem->dma_handle, GFP_KERNEL);
	if (!mem->vaddr) {
			      ...
			      struct videobuf_buffer *buf)
{
	...
	//dma_free_coherent(q->dev, mem->size, mem->vaddr, mem->dma_handle);//nzy delete
	mem->vaddr = NULL;
}
