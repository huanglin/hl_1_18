...
static android_suspend_lock_t alarm_elapsed_realtime_lock ={
	.name =  "android_realtime"
};
...
static void alarm_start_hrtimer(android_alarm_type_t alarm_type)
{
	...
	S_INFO("%s-->%d alarm_type=%d\n",__FUNCTION__,__LINE__,alarm_type);
	if(!(alarm_enabled & (1U << alarm_type)))
		return;
	hr_alarm_time = alarm_time[alarm_type];
	if(alarm_type == ANDROID_ALARM_ELAPSED_REALTIME_WAKEUP)
		android_lock_suspend_auto_expire(&alarm_elapsed_realtime_lock,7*HZ);
	...
	ANDROID_ALARM_DPRINTF(ANDROID_ALARM_PRINT_FLOW, "alarm start hrtimer alarm_type = %d at hr_alarm_time.tv_sec=%ld.hr_alarm_time.tv_nsec%09ld\n", alarm_type, hr_alarm_time.tv_sec, hr_alarm_time.tv_nsec);
	ANDROID_ALARM_DPRINTF(ANDROID_ALARM_PRINT_FLOW, "alarm start hrtimer alarm_type=%d at elapsed_rtc_delta=%ld.%09ld\n", alarm_type, elapsed_rtc_delta.tv_sec, elapsed_rtc_delta.tv_nsec);
...
static long alarm_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	if(alarm_type == 3)
		goto next;
	S_WARN("%s-->%d alarm_type=%d\n",__FUNCTION__,__LINE__,alarm_type);
	S_INFO("%s-->%d ANDROID_ALARM_BASE_CMD(cmd)=%x\n",__FUNCTION__,__LINE__,ANDROID_ALARM_BASE_CMD(cmd));
	S_INFO("%s-->%d ANDROID_ALARM_GET_TIME(0)=%x\n",__FUNCTION__,__LINE__,ANDROID_ALARM_GET_TIME(0));
	S_INFO("%s-->%d ANDROID_ALARM_CLEAR(0)=%x\n",__FUNCTION__,__LINE__,ANDROID_ALARM_CLEAR(0));
	S_INFO("%s-->%d ANDROID_ALARM_SET_AND_WAIT(0)=%x\n",__FUNCTION__,__LINE__,ANDROID_ALARM_SET_AND_WAIT(0));
	S_INFO("%s-->%d ANDROID_ALARM_SET_OLD=%x\n",__FUNCTION__,__LINE__,ANDROID_ALARM_SET_OLD);
	S_INFO("%s-->%d ANDROID_ALARM_SET_AND_WAIT_OLD=%x\n",__FUNCTION__,__LINE__,ANDROID_ALARM_SET_AND_WAIT_OLD);
	S_INFO("%s-->%d ANDROID_ALARM_SET(0)=%x\n",__FUNCTION__,__LINE__,ANDROID_ALARM_SET(0));
	S_INFO("%s-->%d ANDROID_ALARM_WAIT=%x\n",__FUNCTION__,__LINE__,ANDROID_ALARM_WAIT);
	S_INFO("%s-->%d ANDROID_ALARM_SET_RTC=%x\n",__FUNCTION__,__LINE__,ANDROID_ALARM_SET_RTC);
...
next:
	...
	switch(ANDROID_ALARM_BASE_CMD(cmd)) {
		...
		case ANDROID_ALARM_SET_AND_WAIT_OLD:
			...
			rtc_time_to_tm(new_alarm_time.tv_sec, &rtc_new_rtc_time);
			ANDROID_ALARM_DPRINTF(ANDROID_ALARM_PRINT_IO, \
				"case ANDROID_ALARM_SET_AND_WAIT_OLD:\n"
				"set alarm %ld %ld - rtc %02d:%02d:%02d %02d/%02d/%04d\n",
...
from_old_alarm_set:
			spin_lock_irqsave(&alarm_slock, flags);
			ANDROID_ALARM_DPRINTF(ANDROID_ALARM_PRINT_IO, "alarm %d set %ld-%ld \n", alarm_type, new_alarm_time.tv_sec, new_alarm_time.tv_nsec);
			alarm_time[alarm_type] = new_alarm_time;
			alarm_enabled |= alarm_type_mask;
			S_INFO("alarm_enable=%x,alarm_type_mask=%x\n",alarm_enabled,alarm_type_mask);
		...
		case ANDROID_ALARM_WAIT:
			...
			S_INFO("%s-->%d rv=%d alarm_pending=%d\n",__FUNCTION__,__LINE__,rv,alarm_pending);
			if(rv)
				goto err1;
			spin_lock_irqsave(&alarm_slock, flags);
			rv = alarm_pending;
		...
		case ANDROID_ALARM_GET_TIME(0):
			...
			if(alarm_type != ANDROID_ALARM_SYSTEMTIME) {
				...
				ANDROID_ALARM_DPRINTF(ANDROID_ALARM_PRINT_IO,
					"ANDROID_ALARM_GET_TIME  %ld %ld - rtc %02d:%02d:%02d %02d/%02d/%04d\n",
					tmp_time.tv_sec, tmp_time.tv_nsec,
					rtc_new_rtc_time.tm_hour, rtc_new_rtc_time.tm_min,
					rtc_new_rtc_time.tm_sec, rtc_new_rtc_time.tm_mon + 1,
					rtc_new_rtc_time.tm_mday, rtc_new_rtc_time.tm_year + 1900);
				if(alarm_type >= ANDROID_ALARM_ELAPSED_REALTIME_WAKEUP) {
					ANDROID_ALARM_DPRINTF(ANDROID_ALARM_PRINT_FLOW, "ANDROID_ALARM_GET_TIME(0): alarm_type=%d at tmp_time=%ld.%09ld\n", alarm_type, tmp_time.tv_sec, tmp_time.tv_nsec);
					tmp_time = timespec_sub(tmp_time, elapsed_rtc_delta);
					ANDROID_ALARM_DPRINTF(ANDROID_ALARM_PRINT_FLOW, "ANDROID_ALARM_GET_TIME(0): alarm_type=%d at elapsed_rtc_delta=%ld.%09ld\n", alarm_type, elapsed_rtc_delta.tv_sec, elapsed_rtc_delta.tv_nsec);
					ANDROID_ALARM_DPRINTF(ANDROID_ALARM_PRINT_FLOW, "ANDROID_ALARM_GET_TIME(0): alarm_type=%d at tmp_time=%ld.%09ld\n", alarm_type, tmp_time.tv_sec, tmp_time.tv_nsec);
				}
			...
			else
			{
				...
				rtc_time_to_tm(tmp_time.tv_sec, &rtc_new_rtc_time);
				ANDROID_ALARM_DPRINTF(ANDROID_ALARM_PRINT_IO,
					"ANDROID_ALARM_GET_TIME ANDROID_ALARM_SYSTEMTIME  %ld %ld - rtc %02d:%02d:%02d %02d/%02d/%04d\n",
					tmp_time.tv_sec, tmp_time.tv_nsec,
					rtc_new_rtc_time.tm_hour, rtc_new_rtc_time.tm_min,
					rtc_new_rtc_time.tm_sec, rtc_new_rtc_time.tm_mon + 1,
					rtc_new_rtc_time.tm_mday, rtc_new_rtc_time.tm_year + 1900);
...
static enum hrtimer_restart alarm_timer_triggered(struct hrtimer *timer)
{
	...
	S_INFO("%s-->%d alarm_type=%d\n",__FUNCTION__,__LINE__,alarm_type);
	ANDROID_ALARM_DPRINTF(ANDROID_ALARM_PRINT_FLOW, "alarm_timer_triggered type  alarm_type= %d alarm_enabled=%d\n", alarm_type,alarm_enabled);
	spin_lock_irqsave(&alarm_slock, flags);
	S_INFO("alarm_enable=%x,alarm_type_mask=%x\n",alarm_enabled,alarm_type_mask);
	if(alarm_type == ANDROID_ALARM_ELAPSED_REALTIME_WAKEUP)
		android_unlock_suspend(&alarm_elapsed_realtime_lock);
	if (alarm_enabled & alarm_type_mask) {
...
int alarm_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	ANDROID_ALARM_DPRINTF(ANDROID_ALARM_PRINT_FLOW, "alarm_suspend(alarm_enabled=%d, alarm_pending=%d)\n", alarm_enabled, alarm_pending);
	//pr_info("alarm_suspend(alarm_enabled=%d, alarm_pending=%d)\n", alarm_enabled, alarm_pending);
	spin_lock_irqsave(&alarm_slock, flags);
	...
	if (alarm_enabled & ANDROID_ALARM_RTC_WAKEUP_MASK) {
		...
		S_INFO("%s-->%d rtc_current_timespec.tv_sec=%ld rtc_current_timespec.tv_nsec=%ld\n",__FUNCTION__,__LINE__,rtc_current_timespec.tv_sec,rtc_current_timespec.tv_nsec);
		S_INFO("%s-->%d rtc_delta.tv_sec=%ld rtc_delta.tv_nsec=%ld\n",__FUNCTION__,__LINE__,rtc_delta.tv_sec,rtc_delta.tv_nsec);
		S_INFO("%s-->%d elapsed_rtc_delta.tv_sec=%ld elapsed_rtc_delta.tv_nsec=%ld\n",__FUNCTION__,__LINE__,elapsed_rtc_delta.tv_sec,elapsed_rtc_delta.tv_nsec);
		...
		S_INFO("%s-->%d elapsed_realtime_alarm_time.tv_sec=%ld elapsed_realtime_alarm_time.tv_nsec=%ld\n",__FUNCTION__,__LINE__,elapsed_realtime_alarm_time.tv_sec,elapsed_realtime_alarm_time.tv_nsec);
		S_INFO("%s-->%d alarm_time[ANDROID_ALARM_ELAPSED_REALTIME_WAKEUP].tv_sec=%ld alarm_time[ANDROID_ALARM_ELAPSED_REALTIME_WAKEUP].tv_nsec=%ld\n", \
					__FUNCTION__,__LINE__,alarm_time[ANDROID_ALARM_ELAPSED_REALTIME_WAKEUP].tv_sec,alarm_time[ANDROID_ALARM_ELAPSED_REALTIME_WAKEUP].tv_nsec);
		S_INFO("%s-->%d alarm_time[ANDROID_ALARM_RTC_WAKEUP].tv_sec=%ld alarm_time[ANDROID_ALARM_RTC_WAKEUP].tv_nsec=%ld\n", \
					__FUNCTION__,__LINE__,alarm_time[ANDROID_ALARM_RTC_WAKEUP].tv_sec,alarm_time[ANDROID_ALARM_RTC_WAKEUP].tv_nsec);
		   ...
		   || timespec_compare(&alarm_time[ANDROID_ALARM_RTC_WAKEUP], &elapsed_realtime_alarm_time) < 0))
		{
			...
			S_INFO("%s-->%d rtc_alarm_time=%ld\n",__FUNCTION__,__LINE__,rtc_alarm_time);
		}
		else {
			rtc_alarm_time = timespec_sub(elapsed_realtime_alarm_time, rtc_delta).tv_sec;
			S_INFO("%s-->%d(no alarm) rtc_alarm_time=%ld\n",__FUNCTION__,__LINE__,rtc_alarm_time);
		}
		rtc_time_to_tm(alarm_time[ANDROID_ALARM_RTC_WAKEUP].tv_sec, &rtc_alarm.time);
		rtc_alarm.enabled = 1;
		/*pr_info("set alarm  - rtc %02d:%02d:%02d %02d/%02d/%04d week=%02d\n",
		...
		rtc_read_time(alarm_rtc_dev, &rtc_current_rtc_time);
		/*pr_info("Read rtc  - rtc %02d:%02d:%02d %02d/%02d/%04d week=%02d\n",
				rtc_current_rtc_time.tm_hour, rtc_current_rtc_time.tm_min,
				rtc_current_rtc_time.tm_sec, rtc_current_rtc_time.tm_mon + 1,
				rtc_current_rtc_time.tm_mday, rtc_current_rtc_time.tm_year + 1900,rtc_current_rtc_time.tm_wday);*/
		...
		if(rtc_current_time + 1 >= rtc_alarm_time) {
			...
			spin_lock_irqsave(&alarm_slock, flags);
			android_lock_suspend_auto_expire(&alarm_rtc_suspend_lock, 5 * HZ); // trigger a wakeup
...
static int __init alarm_init(void)
{
	...
	if(err < 0)
		goto err3;
	err = android_init_suspend_lock(&alarm_elapsed_realtime_lock);
	if(err < 0)
		goto err4;
	...
	return 0;
err5:
	android_uninit_suspend_lock(&alarm_elapsed_realtime_lock);
err4:
