...
#if 0
#define DBG(fmt, args...)	printk(KERN_INFO "[pm] " "%s(%d): " fmt, __FUNCTION__, __LINE__, ##args)
#else
#define DBG(x...)
#endif
...
static int android_power_debug_mask = ANDROID_POWER_DEBUG_EXIT_SUSPEND ;
...
//#define ENABLE_MODEM_SUSPEND

#ifdef ENABLE_MODEM_SUSPEND
    #ifdef CONFIG_RK2818_HOST11
        extern int modem_is_turn_on();
        extern void turn_off_modem();
        extern void turn_on_modem();
        int modem_need_turn_on = 0;
    #endif
    extern int external_modem_is_turnon();
    extern void turn_on_extern_modem();
    extern void turn_off_extern_modem();
    int extern_modem_need_turn_on = 0;
#endif
#if 0
...
/*20100720 wqq@rk define sleep level (power system)*/
pm_sleep_level_t  g_pm_sleep_level = PM_AWAKE;
pm_sleep_level_t rk2818_get_suspend_flags(void)
{
	DBG("%s->%d leve=%d \n",__FUNCTION__,__LINE__,g_pm_sleep_level);
	return g_pm_sleep_level;
}
pm_sleep_level_t rk2818_set_suspend_flags(pm_sleep_level_t level)
{
	if(level >= PM_ERROR_STATUS)
	{
		DBG("%s->%d leve=%d value  error!\n",__FUNCTION__,__LINE__,level);
		return PM_ERROR_STATUS;
	}
	if(g_user_suspend_state == USER_AWAKE)
	{
		DBG("%s->%d leve=%d  system aleady wakeup successful!!\n",__FUNCTION__,__LINE__,level);
		g_pm_sleep_level = PM_AWAKE;
		return PM_AWAKE;
	}
	#if 0
	if((level == PM_AWAKE)&&(g_user_suspend_state != USER_AWAKE))
	{
		DBG("%s->%d leve=%d  system isn't wakeup successful g_user_suspend_state=%d!!\n",__FUNCTION__,__LINE__,level,g_user_suspend_state);
		return PM_ERROR_STATUS;
	}
	#endif
	//printk("g_user_suspend_state=%d level=%d\n",g_user_suspend_state,level);
	g_pm_sleep_level = level;
	return g_pm_sleep_level;
}
...
static void android_power_wakeup_locked(int notification, ktime_t time)
{
	...
	if (android_power_debug_mask & ANDROID_POWER_DEBUG_EXIT_SUSPEND) {
		...
		pr_info("android_power_request: wakeup (%d->%d) at %lld "
...
static void android_power_request_sleep(void)
{
	...
	if (android_power_debug_mask & ANDROID_POWER_DEBUG_EXIT_SUSPEND) {
		...
		getnstimeofday(&ts);
		rtc_time_to_tm(ts.tv_sec, &tm);
		pr_info("android_power_request: sleep (%d->%d) at %lld "
		       "(%d-%02d-%02d %02d:%02d:%02d.%09lu UTC)\n",
	...
	list_for_each_entry_safe(lock, next_lock, &g_active_full_wake_locks, link) {
		...
		android_unlock_suspend_stat_locked(lock);
	        ...
	        if( (ANDROID_SUSPEND_LOCK_AWAKE&lock->flags) ) {
	                pr_info("%s::lock %s state at full wake\n" , __func__ , lock->name );
	                continue;
	        }
		list_del(&lock->link);
		list_add(&lock->link, &g_inactive_locks);
		pr_info("android_power_suspend: aborted full wake lock %s\n", lock->name);
	}
	spin_unlock_irqrestore(&g_list_lock, irqflags);
	queue_work(g_suspend_work_queue, &g_suspend_work);
}
		...
		if(lock->flags & ANDROID_SUSPEND_LOCK_AUTO_EXPIRE) {
			...
			if(timeout <= 0) {
				...
				if(timeout > max_timeout)
					max_timeout = timeout;
				if(print_locks)
					printk("active wake lock: (%s), time left [%d] jittes\n", lock->name, timeout);
			}
		}
		else {
			if(print_locks)
...
int android_power_driver_suspend_status(void)
{
	return g_user_suspend_state;
}
...
static void android_power_suspend(struct work_struct *work)
{
	...
	while(g_user_suspend_state != USER_AWAKE) {
		...
		if(g_user_suspend_state == USER_NOTIFICATION && list_empty(&g_active_full_wake_locks)) {
			pr_info("android sleep state %d->%d at %lld\n", g_user_suspend_state, USER_SLEEP, ktime_to_ns(ktime_get()));
			g_user_suspend_state = USER_SLEEP;
		}
		spin_unlock_irqrestore(&g_list_lock, irqflags);
		wait = 0;
		if(g_user_suspend_state == USER_AWAKE) {
			pr_err("android_power_suspend: suspend aborted\n");
			return;
		}
		...
		while(g_user_suspend_state == USER_SLEEP) {
			...
			if(g_user_suspend_state != USER_SLEEP)
				break;
			if(g_user_suspend_state != USER_AWAKE)  /*(100811#wqq ) lost wakeup event must reset suspend flag*/
				rk2818_set_suspend_flags(PM_ONELEVEL_SLEEP);
			
#ifdef ENABLE_MODEM_SUSPEND
			if(external_modem_is_turnon()) {
				extern_modem_need_turn_on = 1;
				turn_off_extern_modem();
			}
#endif
...
#ifdef ENABLE_MODEM_SUSPEND
			if(modem_is_turn_on()) {
				modem_need_turn_on = 1;
				turn_off_modem();
			}
#endif
			//rockchip_clk_unlock_pll(SCU_IPID_ARM); // zyy 2010-7-19 
...
#ifdef ENABLE_MODEM_SUSPEND
		if( extern_modem_need_turn_on )
			turn_on_extern_modem();
		extern_modem_need_turn_on = 0;
#ifdef CONFIG_RK2818_HOST11
		if( modem_need_turn_on )
			turn_on_modem();
		modem_need_turn_on = 0;
#endif
#endif
...
static ssize_t state_show(struct kobject *kobj, struct kobj_attribute *attr, char * buf)
{
	...
	spin_unlock_irqrestore(&g_list_lock, irqflags);
	printk("current sleep mode g_pm_sleep_level=%d\n",g_pm_sleep_level);
	return (s - buf);
}
...
static bool gPoweroffFlag = false; 

static bool gKeyIntrruptFlag = false;


static int intrSetCnt = 0;
static int wakeup_intr_set(bool intrDisable)
{	
	if (intrDisable < 0)
		return -EINVAL;

	//printk("%s: Any key wakeup %s\n",__FUNCTION__,intrDisable?"Disable":"Enable");
	if (intrDisable) {
		if (!intrSetCnt) {
			intrSetCnt = 1;
			gpio_irq_disable(RTC_INT_IOPIN);
			gpio_irq_disable(VBUS_INT_IOPIN);
			gpio_irq_disable(IOKEYBOARD_INT0_IOPIN);
			gpio_irq_disable(IOKEYBOARD_INT1_IOPIN);
			gpio_irq_disable(IOKEYBOARD_INT2_IOPIN);
		}
	} else {
		if (intrSetCnt) {
			intrSetCnt = 0;
			gpio_irq_enable(RTC_INT_IOPIN);
			gpio_irq_enable(VBUS_INT_IOPIN);
			gpio_irq_disable(IOKEYBOARD_INT0_IOPIN);
			gpio_irq_disable(IOKEYBOARD_INT1_IOPIN);
			gpio_irq_disable(IOKEYBOARD_INT2_IOPIN);
		}
...
static ssize_t request_state_store(struct kobject *kobj, struct kobj_attribute *attr, const char * buf, size_t n)
{
	...
	if(n >= strlen("idle") &&
	   strncmp(buf, "idle", strlen("idle")) == 0) {
		wakeup_intr_set(gKeyIntrruptFlag);
		android_power_request_sleep();
		return n;
	}
#endif
	if(n >= strlen("standby") &&
	   strncmp(buf, "standby", strlen("standby")) == 0) {
	   	wakeup_intr_set(true);
		android_power_request_sleep();
		return n;
	}
	if(n >= strlen("wake") &&
	   strncmp(buf, "wake", strlen("wake")) == 0) {
		wakeup_intr_set(false);
		android_power_wakeup();
		return n;
	}
	if(n >= strlen("poweroff") &&
	   strncmp(buf, "poweroff", strlen("poweroff")) == 0) {
		gPoweroffFlag = true;
		printk("request_state_store gPoweroffFlag=%d\n",gPoweroffFlag);
		return n;
	}
	//printk("%s:[%s] invalid argument\n",buf,__FUNCTION__);
	return -EINVAL;
}
...
static ssize_t acquire_partial_wake_lock_store(struct kobject *kobj, struct kobj_attribute *attr, const char * buf, size_t n)
{
	...
	if (android_power_debug_mask & ANDROID_POWER_DEBUG_USER_WAKE_LOCK)
		printk(KERN_INFO "acquire_partial_wake_lock_store: %s, "
			"size %d  timeout=%d \n", g_user_wake_locks[i].name_buffer, n,timeout);
...
static ssize_t standby_mode_store(struct kobject *kobj,
				struct kobj_attribute *attr, const char * buf, size_t n)
{
	if(n >= strlen("standby") &&
		strncmp(buf, "standby", strlen("standby")) == 0) {
		gKeyIntrruptFlag = true;
		return n;
	}
	if(n >= strlen("idle") &&
		strncmp(buf, "idle", strlen("idle")) == 0) {
		gKeyIntrruptFlag = false;
		return n;
	}

	printk("%s:%d: invalid argument\n",__FUNCTION__,__LINE__);
	return -EINVAL;
}
static ssize_t standby_mode_show(struct kobject *kobj,
				     struct kobj_attribute *attr, char *buf)
{
	char * s = buf;

	if (gKeyIntrruptFlag)
		s += sprintf(buf, "standby\n");
	else
		s += sprintf(buf, "idle\n");
...
android_power_attr(standby_mode);
#ifndef CONFIG_FRAMEBUFFER_CONSOLE
