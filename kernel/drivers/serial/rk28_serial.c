...
#define UART_GET_MCR(port)	    __raw_readl(&(pheaduart->UART_MCR))            
#define UART_PUT_MCR(port,v)	__raw_writel(v,&(pheaduart->UART_MCR))          
#define UART_PUT_FCR(port,v)	__raw_writel(v,&(pheaduart->UART_FCR))             
#define UART_GET_FCR(port)	    __raw_readl(&(pheaduart->UART_FCR))           
#define UART_GET_MSR(port)	    __raw_readl(&(pheaduart->UART_MSR)) 
...
{	
	...
	if(pending == THR_EMPTY)
		rockchip_serial_start_tx(port);		
	return IRQ_HANDLED;	
}
			      ...
			      struct ktermios *old)
{
    ...
    //spin_lock_irqsave(&port->lock, flags);
	    ...
	    {        
			/*开启uart0硬件流控*/
			rockchip_mux_api_set(GPIOB2_U0CTSN_SEL_NAME, IOMUXB_UART0_CTS_N);
			rockchip_mux_api_set(GPIOB3_U0RTSN_SEL_NAME, IOMUXB_UART0_RTS_N);
			printk("start CRTSCTS control and baudrate is %d\n",baud);
			umcon=UART_GET_MCR(port);
			printk("UART_GET_MCR umcon=0x%x\n",umcon);
			umcon |= UART_MCR_AFCEN;
			umcon |= UART_MCR_URRTS;
			umcon &=~UART_SIR_ENABLE;
			UART_PUT_MCR(port,umcon);
			printk("UART_GET_MCR umcon=0x%x\n",umcon);
			fcr=UART_GET_FCR(port);
    ...
    #else
    while( !(UART_TRANSMIT_FIFO_EMPTY&UART_GET_USR(port)) ) {
                mdelay( 1 );
                cpu_relax();
    }
    #endif
