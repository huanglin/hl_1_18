...
struct lbs_private *lbs_add_card(void *card, struct device *dmdev)
{
	...
	/*
	 * We needn't support MULTICAST. Yongle Lai
	 */
...
#if (ANDROID_POWER_SAVE == 1)
    /* 20091110,HSL@RK */
    //printk("%s::wifi priv=0x%p,ps timer=0x%p,ps work=0x%p" , __func__ , 
    //	   lbs_wifi_priv , &lbs_wifi_priv->ps_timer , &lbs_wifi_priv->ps_work);
#endif
...
void lbs_stop_card(struct lbs_private *priv)
{
	...
	//printk("command timer is deleted.\n");
