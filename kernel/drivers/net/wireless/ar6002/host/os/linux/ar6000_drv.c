...
AR_SOFTC_T                  *ar6_wifi_priv = NULL;
int                         wifi_ps_status = WIFI_PS_AWAKE;
#endif
...
char *iic_data = FWPATH"iic.data";
char *iic_bin  = FWPATH"iic.bin";

extern int wifi_external_eeprom;
...
void ar6000_update_ps_timer(void)
{
    AR_SOFTC_T  *ar = ar6_wifi_priv;
    ...
    if (ar->ps_timer.function == NULL)
       return;

    printk("%s\n",__func__);

    mod_timer(&ar->ps_timer,jiffies + DRIVER_PS_TIMEOUT);
}
#endif
...
int wifi_real_suspend(int state)
{
    ...
    printk("%s: WIFI Disconnect because of PM.\n",__func__);
    ...
    if (status != A_OK)
	{
		printk("Send Disconnect Command Failed(status = %d).\n", status);
...
void wifi_driver_resume(void)
{
	...
	if (ar == NULL)
	{
		printk("ar6_wifi_priv is NULL.\n");
		return;
	}
#if 0
	if ((wifi_ps_status != WIFI_PS_DRV_SLEEP) ||
        (wifi_ps_status != WIFI_PS_SYS_SLEEP))
	{
	 ...
	 * Update timer.
	 */
    ar6000_update_ps_timer();

    reconnect_flag = true;
    wifi_ps_status = WIFI_PS_PRE_WAKE;
    
    status = wmi_reconnect_cmd(ar->arWmi,ar->arReqBssid,ar->arChannelHint);
    if (status != A_OK)
	{
		printk("Send Reconnect command failed, status = %d.\n", status);
		wifi_ps_status = WIFI_PS_DRV_SLEEP;
        return;
	}
...
void ath_ps_worker(struct work_struct *work)
{
    AR_SOFTC_T  *ar = ar6_wifi_priv;
	
	printk("%s().\n",__func__);
	
    if(ar->wifi_ps_work_req ==  WIFI_PS_PRE_WAKE)
		wifi_driver_resume();
	else
		wifi_driver_suspend(0);
	
	ar->wifi_ps_work_req = WIFI_PS_AWAKE;
}


static void ps_timer_fn(unsigned long data)
{
    AR_SOFTC_T *ar = (AR_SOFTC_T *)data;
    ...
    ar->wifi_ps_work_req = WIFI_PS_DRV_PRE_SLEEP;

	queue_delayed_work(ar->ps_work_thread, &ar->ps_work,
					   msecs_to_jiffies(5));
}
...
/* ATHENV */
    ...
    //printk("init insmode driver +2\n");


    //printk("init insmode driver +3\n");
...
/* ATHENV */
    ...
    /* Init sdio host and register sdio function driver. */
...
/**********************hhy******************/
#if (ANDROID_POWER_SAVE == 1)
    printk("Remove ps timer.\n");
    
    del_timer_sync(&ar->ps_timer);

    if (ar->ps_work_thread != NULL)
	    destroy_workqueue(ar->ps_work_thread); 
...
/* ATHENV */
    ...
    if (wifi_power_control == 0) //没有IO控制，我们不需要控制。
        return;
...
static void firmware_transfer(HIF_DEVICE *device, char* filename, A_UINT32 address, A_BOOL isCompressed)
{
    ...
    while (remaining>0) {
        ...
        address += length;
                ...
                //printk("START: second=%ld usec=%ld\n", sdio_start_tv.tv_sec, sdio_start_tv.tv_usec);
                //printk("STOP : second=%ld usec=%ld\n", sdio_stop_tv.tv_sec, sdio_stop_tv.tv_usec);
                ...
                printk("Data size = %d bytes, Time = %lu.%lu seconds, Rate = %lu kbps\n",
                        length2, download_second, download_time,
                        (length2 * 8 * 1000) / (download_second * 1000 + download_time) / 1000);
...
/* ATHENV */
    ...
    setup_timer(&ar->arHBChallengeResp.timer, ar6000_detect_error, (unsigned long)dev);
    ...
    setup_timer(&aptcTimer, aptcTimerHandler, (unsigned long)ar);
#endif /* ADAPTIVE_POWER_THROUGHPUT_CONTROL */

    //A_INIT_TIMER(&ar->disconnect_timer, disconnect_timer_handler, dev);
    setup_timer(&ar->disconnect_timer, disconnect_timer_handler, (unsigned long)dev);
...
/********************hhy**********************/
    ...
    ar->ps_work_thread = create_singlethread_workqueue("a6k_ps_worker");

    INIT_DELAYED_WORK(&ar->ps_work, ath_ps_worker);

    setup_timer(&ar->ps_timer, ps_timer_fn, (unsigned long)ar);

    mod_timer(&ar->ps_timer,jiffies + DRIVER_PS_TIMEOUT + 40*HZ);
...
/* ATHENV */
	...
	if (wifi_external_eeprom == 1)
	{
    	firmware_transfer(ar->arHifDevice, iic_data, 0x502070, FALSE);
    	firmware_transfer(ar->arHifDevice, iic_bin, 0x513950, FALSE);
...
/*********************hhy***************/
    ...
    {
        if(ar->wifi_ps_work_req == WIFI_PS_AWAKE)
        {
            printk("Need to wakeup for data tx.\n");
            ar->wifi_ps_work_req = WIFI_PS_PRE_WAKE;
...
#if (ANDROID_POWER_SAVE == 1)
	if ((wifi_ps_status >= WIFI_PS_PRE_SLEEP) && 
	    (wifi_ps_status <= WIFI_PS_DRV_SLEEP))
	{
	    printk("Drop the disconnect event while in system deepsleep.\n");
		return;
	}
#endif
