...
android_suspend_lock_t rtl8188_suspend_lock;     
#else
...
//when return 0 is busy 
int rtl8188_power_state(void)
{
	if(0 != rtl8188_wakelock_init)
		return 0;//busy	
	else
		return 1;//idle
}
...
{
#ifdef CONFIG_ANDROID_POWER
	rtl8188_suspend_lock.name = RTL8188_WAKELOCK_NAME;
	android_init_suspend_lock(&rtl8188_suspend_lock);
	android_lock_suspend(&rtl8188_suspend_lock);
#else
...
{
	msleep(100);
	printk("%s rtl8188_wakelock_init :%d\n",__FUNCTION__,rtl8188_wakelock_init);
	if (rtl8188_wakelock_init == 2)
#ifdef CONFIG_ANDROID_POWER
		android_unlock_suspend(&rtl8188_suspend_lock);
#else
		wake_unlock(&rtl8188_suspend_lock);
#endif

#ifdef CONFIG_ANDROID_POWER
	android_uninit_suspend_lock(&rtl8188_suspend_lock);
#else
