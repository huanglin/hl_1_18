...
void _I2CResetStart(eI2C_ch_t i2cNumb)
{
	int32 waiteDelay;
	pI2C_REG  pI2cRegDev;
	...
	pI2cRegDev->I2C_LCMR = pI2cRegDev->I2C_LCMR | I2C_LCMR_RESUME | /*(I2C_LCMR_STOP*/I2C_LCMR_START;

	while (((pI2cRegDev->I2C_LCMR & /*I2C_LCMR_STOP*/I2C_LCMR_START) != 0)&& (waiteDelay > 0))
	{
		udelay(100*10);
		waiteDelay--;
	}
...
#else
int _I2CUpdateAllApbFreq(ip_id ip,int APBHz , int stage )
{
                if( stage != STAGE_AFTER_CHG )
                        return 0;
	APBHz = APBHz/1000; /* hz to khz */
                S_INFO("%s:set I2C%d to %d KHZ\n" , __func__, ip-SCU_IPID_I2C0,APBHz);
	if(g_i2cSpeed0 > 0 && ip == SCU_IPID_I2C0 )
	{
		_I2CSetSpeed(I2C_CH0,g_i2cSpeed0,APBHz);
	}
...
//fs_initcall_sync(rk28_i2c_init);
arch_initcall_sync(rk28_i2c_init);
