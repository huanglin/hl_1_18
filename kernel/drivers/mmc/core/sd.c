...
static int mmc_decode_scr(struct mmc_card *card)
{
	...
	unsigned int scr_struct;
	u32 resp[4];
    u8  *pDataBuf = (u8 *)&resp[2];

	resp[3] = card->raw_scr[1];
	resp[2] = card->raw_scr[0];
    //printk("%s..%s..%d *********card->raw_scr[0]= %8x, card->raw_scr[1]=%8x*********====xbw===*******************\n",__FUNCTION__,__FILE__,__LINE__, resp[2], resp[3]);

	scr_struct = ((pDataBuf[7] & 0xF0)>>4);//scr_struct = UNSTUFF_BITS(resp, 60, 4);
	if (scr_struct != 0) {
		printk(KERN_ERR "%s: unrecognised SCR structure version %d\n",
			mmc_hostname(card->host), scr_struct);
		return -EINVAL;
    ...
    //printk("%s..%s..%d ******pDataBuf[7]=%x****** 2 ***====xbw===*******************\n",__FUNCTION__,__FILE__,__LINE__, (pDataBuf[7] & 0x0f));
    
	scr->sda_vsn = (pDataBuf[7] & 0x0f);//scr->sda_vsn = UNSTUFF_BITS(resp, 56, 4);
	
	scr->bus_widths = (pDataBuf[6] & 0x4);//scr->bus_widths = UNSTUFF_BITS(resp, 48, 4);
    //printk("%s..%s..%d *******scr->sda_vsn=%x **scr->bus_widths=%x****** 2 ***====xbw===*******************\n",__FUNCTION__,__FILE__,__LINE__, scr->sda_vsn, scr->bus_widths);
	...
	struct mmc_card *oldcard)
{
    ...
    extern android_suspend_lock_t sdmmc0_initSd_lock;

    #ifdef CONFIG_ANDROID_POWER	
    android_lock_suspend(&sdmmc0_initSd_lock);
    #endif    
...
Retry_init:
   ...
   // printk("\n mmc_sd_init_card***** CID提取正确 ********xbw***********\n");
	...
	if (mmc_card_highspeed(card)) {
        ...
        //printk("\n mmc_sd_init_card***** SD  select HighSpeed, max_dtr=%d ********xbw***********\n", max_dtr);
...
#ifdef CONFIG_ANDROID_POWER	
    android_unlock_suspend(&sdmmc0_initSd_lock);
#endif 
...
int mmc_attach_sd(struct mmc_host *host, u32 ocr)
{
	...
	mmc_release_host(host);
     ...
     printk("%s..%s..%d - Now, call mmc_add_card to add a new card.\n",__FUNCTION__,__FILE__,__LINE__);
