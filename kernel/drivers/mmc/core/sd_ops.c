...
int mmc_send_app_op_cond(struct mmc_host *host, u32 ocr, u32 *rocr)
{
   ...
   // printk("%s..%d *********Before CMD41, cmd.arg = %x **********====xbw===*******************\n",__FUNCTION__,__LINE__, cmd.arg);
	...
	//for (i = 100; i; i--) 
	for (i = 300; i; i--)  //卡初始化阶段，这个等待阶段可以适当加长时间；但是，后面 mmc_wait_for_app_cmd出错，3次已经足够。若错了，再重试也一般是错。 xbw@2010-05-05
	{
...
#if 1 //Delted by xbw@2009-07-07; Linux intentionally set the data to big-endian.
    else  //hcy
	{
       ...
       /*
	    for(i=0; i<8; i++)
	    {
            printk("\n mmc_app_send_scr：pDataBuf[%d]:%x  *********xbw********\n", i, pDataBuf[i]);
...
#if 0 //the data of CMD6 can't convert. we should moidfy the judgement about the content at the same time.; Deleted by xbw
	else  //hcy
	{
