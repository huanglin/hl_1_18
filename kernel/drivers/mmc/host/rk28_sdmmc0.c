...
static void rk28_sdmmc0_set_ios(struct mmc_host *mmc, struct mmc_ios *ios)
{
    ...
    {
        /*
        //Begin to use the internal divider, so not to use external divider. Deleted by xbw@2010-07-19
        //set the scu mode
       ...
       */
        printk("%s..%s..%d ***********The clock div is setted. the ios->clock=%d Khz******====xbw===******\n",__FUNCTION__,__FILE__,__LINE__, ios->clock/1000);
    ...
    if ((ios->bus_width == MMC_BUS_WIDTH_4))
    {
        ...
        //printk("%s..%s..%d ***********SDMMC0 Set the 4-line data*****====xbw===******\n",__FUNCTION__,__FILE__,__LINE__);
    }
    else
    {
...
int sd_cdt_resume = 0;
int sd_is_suspend_block = 0;
int sd_is_suspend_mmc=0;
int do_Resume = 0;
