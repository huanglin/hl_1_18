...
bool   SDPAM_IOMUX_SetSDPort(struct rk28mci_host *host)
{
    ...
    switch (host->busWidth)
    {
		...
		GPIOPullUpDown(GPIOPortH_Pin3,GPIONormal);
...
int32 rk28_mci_ChangeFreq(struct rk28mci_host *host, uint32 freqKHz)
{
    ...
    //printk("%s..%d ***ahb-card=%d*,  AHB=%d*****====xbw===***\n",__FILE__,__LINE__, ahbfrq, rockchip_clk_get_ahb());
    
    if (freqKHz <= 400)
    {
        suitCclkInDiv = (ahbfrq/freqKHz) + ((( ahbfrq%freqKHz ) > 0) ? 1:0);     
    }
    else if((freqKHz < 25500) && (ahbfrq > freqKHz))
    {          
        //printk("%s..%d *****freqKHz= %d , so use the internal div****====xbw===***\n",__FILE__,__LINE__, freqKHz);
        suitCclkInDiv = (ahbfrq/freqKHz) + ((( ahbfrq%freqKHz ) > 0) ? 1:0);     
    }
    else
    {
...
static int32 rk28_PrepareForWriteData(struct rk28mci_host *host, struct mmc_data *data)
{
    ...
    else
    {
        ...
        {  
          //
          //Because the IO-Mode is an inefficient way, so it is best to avoid using IO-mode.
          //
          
          //  printk("%s..%d *****desLen = %d, write by IO-type, CLK=%dMHz, dataAddr=%x,CMD=%d******====xbw===*****\n",
           //     __FILE__,__LINE__, count<<2, host->iosclock/1000000, pBuf, host->cmd->opcode);
            ...
            if (count > FIFO_DEPTH)
            {
                ...
                host->intInfo.transLen = FIFO_DEPTH;
            }
            else
            {
                for (i=0; i<count; i++)
                ...
                host->intInfo.transLen = count;
            }
           
            return SDC_SUCCESS;
        } 
        else if ((output = rk28_dma_request(host->dma,  rk28_mci_dma_irq, host))!=0)
        {
            host->requestDmaError = 1;//   request error.
             printk("%s..%d ***DMA request ERROR in WriteCMD*** dmaErr=%d,  now try to transfer data by IO-type******====xbw===***\n",__FILE__,__LINE__, output);
            
            #if 0//Use_SD_DMA
            return  SDC_FALSE;
            #else
            //dynamic use DMA; Modifyed xbw@2010-07-08
            value |= (RXDR_INT | TXDR_INT);    
            rk28_mci_write(host, SDMMC_INTMASK, value);//set INTMASK each request
    ...
    if (count > (RX_WMARK+1))  //datasheet error.actually, it can nont waken the interrupt when less and equal than RX_WMARK+1
    {
        if(0) //(host->intInfo.desLen <= 512 )
        {
          ...
          //
           ...
           // printk("%s..%d *****desLen = %d, read by IO-type, sdCLK=%dMHz,  dataAddr=%x******====xbw===**********\n",
           //     __FILE__,__LINE__, host->intInfo.desLen<<2, host->iosclock/1000000, host->pbuf);
        ...
        {
            host->requestDmaError = 1;//   request error.
            printk("%s..%d *****DMA request ERROR in ReadCmd***  dmaErr=%d,  now try to transfer data by IO-type******====xbw===**********\n",__FILE__,__LINE__, output);
        ...
        if (0 != output)
        {
            printk("%s..%d ***********DMA setup ERROR in readCmd,*** dmaErr=%d******====xbw===*******************\n", __FILE__,__LINE__,output);
            return SDC_FALSE;
        }
...
int32 rk28_ReadRemainData(struct rk28mci_host *host, uint32 originalLen, void *pDataBuf)
{
    ...
    else
    {
        if (!((value = rk28_mci_read(host, SDMMC_STATUS)) & FIFO_EMPTY))
        {
             while ( (host->intInfo.transLen < host->intInfo.desLen)  && (!(value & FIFO_EMPTY)) )
            {
                pBuf[host->intInfo.transLen++] = rk28_mci_read(host, SDMMC_FIFO);  
                value = rk28_mci_read(host, SDMMC_STATUS);  
            }
    ...
    //printk("%s..%s..%d *********End the transfer, desLen=%d, traslen=%d*****====xbw===*****\n",__FUNCTION__,__FILE__,__LINE__,host->intInfo.desLen,host->intInfo.transLen);

    return SDC_SUCCESS;
}


static void do_pio_read(struct rk28mci_host *host)
{
    int i;
    for (i=0; i<(RX_WMARK+1); i++)
    {
        host->intInfo.pBuf[host->intInfo.transLen + i] = rk28_mci_read(host, SDMMC_FIFO);
    }
    host->intInfo.transLen += (RX_WMARK+1);
}

static void do_pio_write(struct rk28mci_host *host)
{
    int i;
    if ( (host->intInfo.desLen - host->intInfo.transLen) > (FIFO_DEPTH - TX_WMARK) )
    {
        ...
        {
            rk28_mci_write(host, SDMMC_FIFO, host->intInfo.pBuf[host->intInfo.transLen + i]);
        }
    ...
    //printk("%s....%s....%d   ******desLen =%d, transLen=%d************* xbw*******\n",__FUNCTION__,__FILE__,__LINE__, host->intInfo.desLen, host->intInfo.transLen);
}
...
void rk28_sdmmc_setup_data(struct rk28mci_host *host, struct mmc_data *data)
{
    ...
    if (data)
    {
        ...
        { 
            /*
            //Debug info
            if(host->cmd->opcode == 25)
            {
                printk("%s..%d *****Multi write CMD=%d, desLen = %d, dataAddr=%x******====xbw===**********\n",
                    __FILE__,__LINE__, host->cmd->opcode, data->blocks*data->blksz,  host->cmd->arg);
             }
             else
             {
                 printk("%s..%d *****     write CMD=%d, desLen = %d, dataAddr=%x******====xbw===**********\n",
                    __FILE__,__LINE__, host->cmd->opcode, data->blocks*data->blksz,  host->cmd->arg);
             }
              */   
        ...
        else
        {
            /*
            //Debug info
            if(host->cmd->opcode == 18)
            {
                printk("%s..%d *****Multi Read=== CMD=%d, desLen = %d, dataAddr=%x******====xbw===**********\n",
                    __FILE__,__LINE__, host->cmd->opcode, data->blocks*data->blksz,  host->cmd->arg);
            }
            else
            {
                printk("%s..%d *****      Read=== CMD=%d, desLen = %d, dataAddr=%x******====xbw===**********\n",
                    __FILE__,__LINE__, host->cmd->opcode, data->blocks*data->blksz,  host->cmd->arg);
            }
             */    
...
uint32 rk28_sdmmc_start_command(struct rk28mci_host *host,struct mmc_command *cmd)
{
    ...
    if((1==host->requestDmaError) && (1==host->complete_dma))
    {
    ...
    printk("%s..%s..%d ***** CMD=%d,  cmdr = %x , arg = %x=*********==xbw=****************\n",__FUNCTION__,__FILE__,__LINE__,cmd->opcode, cmdr, cmd->arg );
#endif
...
StarCMDError_exit:
    printk("%s..%s..%d *******start cmd%d error====errorflag=%d==xbw=****************\n",__FUNCTION__,__FILE__,__LINE__, 
    host->cmd->opcode,errorflag);
...
extern int sd_cdt_resume;
extern int sd_is_suspend_block;
extern int sd_is_suspend_mmc;
...
irqreturn_t rk28_sdmmc_irq(int irq ,  void *devid)
{
    ...
    if (value & CDT_INT)
    {
        ...
        {        
            ...
            if(sd_is_suspend_block || sd_is_suspend_mmc) 
            {
                sd_cdt_resume = 1;
				//sd_cdt_block_resume = 1;
				sd_is_suspend_block = 0; //cleard by resume subroutine
				sd_is_suspend_mmc = 0;
    ...
    if (!host->request)
    {
        printk("%s....%s....%d   ******requestNULL , hardware have sickness.Please check hardware.********xbw*******\n",__FUNCTION__,__FILE__,__LINE__);
        rk28_mci_write(host, SDMMC_RINTSTS, 0xFFFFFFFF);
    ...
    if(0 == host->complete_dma)
    {
        if (value & RXDR_INT)
        {
            //read data.
            //printk("%s....%s....%d   **********RXDR_INT value=%x ***********  xbw  **************\n",__FUNCTION__,__FILE__,__LINE__, value);
            do_pio_read(host);
            rk28_mci_write(host, SDMMC_RINTSTS, RXDR_INT);
        }
        else if (value & TXDR_INT)
        {
            //fill data
            //printk("%s....%s....%d   **********TXDR_INT value=%x *********  xbw  **************\n",__FUNCTION__,__FILE__,__LINE__, value);
            do_pio_write(host);
            rk28_mci_write(host, SDMMC_RINTSTS, TXDR_INT);
        }
    }
...
/*
        if(rk28_sdmmc_IsCardPresence(host))
        {
            ...
            local_irq_disable();
            while(1); //hold the point, when error happen            
        }*/
    ...
    if (cmdr & DATA_EXPECT) 
    {
        ...
        //printk("%s....%d   *****data transfer over**dstlen=%d, translen=%d********  xbw  **************\n",__FILE__,__LINE__, host->intInfo.desLen, host->intInfo.transLen);
        if (waitTime == 0 )
        {
            cmd->error = -EIO;
            host->cmderror = cmd->error;            
            ...
            errorflag = 12;
            //goto exit; //not exit, but continue to do the remain thing.; xbw@2010-04-13
        }
        ...
        if (cmdr & WRITE_CARD)
        {
            ...
            else
            {
                ...
                if (ret != SDC_SUCCESS)
                {
                    ...
                    errorflag = 14;
                    SDMMC_printk("%s....%s....%d   **********fail_task ***********  xbw  **************\n",__FUNCTION__,__FILE__,__LINE__);
                    goto exit; //It can exit if it is busy.;xbw@2010-04-13
                }
        ...
        else
        {
            ...
            if (value & SBE_INT)
            {
            #if 1 //It's important to next request,though not useful for current request.
                ...
                if(0==host->complete_dma)
                {
                    value = (SBE_INT | HLE_INT | FRUN_INT | DTO_INT |CDT_INT | RXDR_INT | TXDR_INT);
                }
                else
                {
                    value = (SBE_INT | HLE_INT | FRUN_INT | DTO_INT |CDT_INT);
...
exit:
    if(SD_CMD5 != cmd->opcode)
    {
        //printk("%s,errorflag=%d,cmd=%d,cmdr=%x,ret=0x%x\n",__FUNCTION__, errorflag, cmd->opcode,host->cmdr , ret );
    }
...
void rk28_sdmmc_send_request(struct mmc_host *mmc)
{
    ...
    {
        printk("%s..%d*******host->request=Null, Hardware have sickness*************", __FILE__,__LINE__ );
        return;
    }
#endif
    host->flags = 0;
    ...
    if(0==host->complete_dma)
    {
        ...
        value = (SBE_INT | HLE_INT | FRUN_INT | DTO_INT |CDT_INT ); //restore to default state that card use DMA
        rk28_mci_write(host, SDMMC_INTMASK, value);
    }
