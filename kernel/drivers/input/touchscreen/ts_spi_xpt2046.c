...
static int xpt2046_read_values(struct XPT2046_TS_EVENT *ts_dev)
{
	...
	ts_spi_printk("pressure = %d	adx = %d	ady = %d\n",gDebugData[0],gDebugData[1],gDebugData[2]);
	ts_spi_printk("pressure = %d\n",gDebugData[0]);
	kfree(ptmpdata);
	return ret;
}
...
static void  xpt2046_send_values(struct XPT2046_TS_EVENT *ts_dev)
{
	...
	}else		/*valid event*/
	{
		...
		if(!ts->pendown)
		{
			ts_spi_printk("The touchscreen down!!\n");
...
static	int __devexit xpt2046_ts_remove(struct spi_device *pdev)
{
	...
	ts_spi_printk("*****************************xpt2046_ts_remove******************\n");
...
static int __init xpt2046_ts_init(void)
{
	...
	ts_spi_printk("Touch panel drive XPT2046 driver init...\n");
...
{
	ts_spi_printk("Touch panel drive XPT2046 driver exit...\n");
