...
static int rkusb_complete_out(struct usb_ep *ep, struct usb_request *req)
{
                ...
                //S_INFO("%s::ep=0x%p,f0 ep=0x%p,f1 ep=0x%p\n" , __func__ , ep , 
                //         dev->f[0].out, dev->f[1].out);
                        
                if( dev->phase == RKUSB_PHASE_CMD ) {
                        cb = (struct rkusb_cb_wrap*)req->buf;
                        /* 20100408,HSL@RK,from ADB to RKUSB,out req length may == 512. */
                        if ( cb->Signature != __constant_cpu_to_le32(RKUSB_BULK_CB_SIG)
                                || ( req->actual != RKUSB_BULK_CB_WRAP_LEN  
                                && ((int)req->actual)  != RKUSB_BULK_CB_WRAP_LEN-512)) {
                        ...
                        // XXX: 20101229,HSL@RK,if adb and msc command go together , 
                        // from the msc endpoint ?
                        if( dev->f[1].out == ep ) {
                                f = 1;
                                DEV_OFFSET(dev) = __be32_to_cpu( DEV_OFFSET(dev) );
                        } 
                        dev->ep_in = dev->f[f].in;
                        dev->req_in = dev->f[f].req_in;

                        S_INFO("%s::got msc cmd at interface %d,cmd=0x%x\n" , __func__ , f , cb->Code );
