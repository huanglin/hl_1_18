		...
		unsigned intf, unsigned alt)
{
	...
	wake_up(&dev->read_wq);
	RKUSB_OPEN(dev->ep_in,dev->ep_out,0);	
	adb_rkinit( dev );
	return 0;
}
...
int __init adb_function_add(struct usb_configuration *c)
{
	...
	dev->function.forbid = 1; /* default:close */
	...
	if (ret)
        ...
        //adb_support = 1;
                //adb_function_enable( 1 ); /* 20100127,HSL@RK,for test,20100423,CLOSE as default.*/
	return 0;
