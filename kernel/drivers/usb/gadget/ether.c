 ...
 */
//hjk
#define CDC_VENDOR_NUM	0x230b
//0x0525		/* NetChip */
#define CDC_PRODUCT_NUM	0x0001
...
static int eth_start_xmit (struct sk_buff *skb, struct net_device *net)
{
	...
	if (gadget_is_dualspeed(dev->gadget))
		req->no_interrupt = (dev->gadget->speed == USB_SPEED_HIGH)
			...
			: 0;
	if(((unsigned int)req->buf) & 3){
		if(eth_tx_buff==0)
			{
				eth_tx_buff=kmalloc(req->length, GFP_KERNEL);
				eth_tx_buff_length = req->length;
			}
		if(eth_tx_buff_length < req->length)
			{
				kfree(eth_tx_buff);
				eth_tx_buff=kmalloc(req->length, GFP_KERNEL);
				eth_tx_buff_length = req->length;
		...
		memcpy(eth_tx_buff,req->buf,eth_tx_buff_length);
		req->buf = eth_tx_buff;
	}
