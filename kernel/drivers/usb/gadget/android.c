...
static void enable_adb(struct android_dev *dev, int enable)
{
	if (enable != dev->adb_enabled) {
        ...
        #endif
        if(enable && !memcmp(strings_dev[STRING_SERIAL_IDX].s, UNKNOWN_SN, sizeof(UNKNOWN_SN))) {
            char idbdata[IDBDATA_SIZE];
            memset(idbdata, 0, IDBDATA_SIZE);
            
            GetSNSectorInfo(idbdata);
            unsigned short size =*((unsigned short *) idbdata);
            if(size > 0 && size <= SN_SIZE) {
                idbdata[size+2] = '\0';
                memcpy((void *)strings_dev[STRING_SERIAL_IDX].s, idbdata+2, SN_SIZE);
            
                printk("android set serial number:%s\n", strings_dev[STRING_SERIAL_IDX].s);
            }
        }
    }
}
...
static int __init android_probe(struct platform_device *pdev)
{
	...
	if (pdata) {
        ...
        char * serial_number = kzalloc(SN_SIZE, GFP_KERNEL);
        memset(serial_number, 0, SN_SIZE);
        memcpy(serial_number, UNKNOWN_SN, sizeof(UNKNOWN_SN));
...
static void __exit cleanup(void)
{
    ...
    platform_driver_unregister(&android_platform_driver);
    kfree(_android_dev);
    kfree(strings_dev[STRING_SERIAL_IDX].s);
    _android_dev = NULL;
}
module_exit(cleanup);
