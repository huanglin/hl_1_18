...
//#define RK28_WM8988_DBG
#ifdef RK28_WM8988_DBG
#define printk_d(fmt, args...)	printk(KERN_INFO "[RK28_WM8988] " "%s(%d): " fmt, __FUNCTION__, __LINE__, ##args)
#else
...
{    
	...
	//ret = cpu_dai->dai_ops.set_sysclk(cpu_dai, 0, 75000000, SND_SOC_CLOCK_IN);
	//if (ret < 0)
	//	return ret;

	/* codec system clock is supplied by PCK1, set to 12MHz */
	ret = codec_dai->dai_ops.set_sysclk(codec_dai, 0, 12000000, SND_SOC_CLOCK_IN);
	if (ret < 0)
		return ret;
