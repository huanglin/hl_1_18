/*
 * rockchip_wm8904.c  --  SoC audio for rockchip
 *
...
#if 0
#define printk_d(fmt, args...)  printk(KERN_INFO "[RK-WM8904] " "%s(%d): " fmt, __FUNCTION__, __LINE__, ##args)
#else
...
{    
    ...
    printk_d("Enter::%s----%d\n",__FUNCTION__,__LINE__);   
	  ...
	  ret = codec_dai->dai_ops.set_sysclk(codec_dai, 0,  ///WM8904_SYSCLK,
...
static const char *audio_map[][3] = {
	...
	/* mic is connected to Mic Jack, with WM8904 Mic Bias */
	{"MICIN", NULL, "Mic Bias"},
 ...
 */
static int rockchip_wm8904_init(struct snd_soc_codec *codec)
{
	int i;
...
static struct snd_soc_dai_link rockchip_dai = {
	  ...
	  .init = rockchip_wm8904_init,
	  .ops = &rockchip_ops,
};

static struct snd_soc_machine snd_soc_machine_rockchip = {
	  .name = "ROCKCHIP_WM8904",
	  .dai_link = &rockchip_dai,
	  .num_links = 1,
};

static struct wm8904_setup_data rockchip_wm8904_setup = {
	  .i2c_address = 0x1a,
};
...
static struct snd_soc_device rockchip_snd_devdata = {
	  ...
	  .codec_data = &rockchip_wm8904_setup,
};
...
static int __init audio_card_init(void)
{
    ...
    if (!request_mem_region(I2S_BASE_ADDR, 0x20, "soc-audio")) {
	  	  printk_d("I2S memory region is busy\n");
	  	  return -EBUSY;
	  }
	  rockchip_snd_device = platform_device_alloc("soc-audio", -1);
	  if (!rockchip_snd_device) {
	  	  printk_d("platform device allocation failed\n");
	  	  ret = -ENOMEM;
	  ...
	  if (ret) {
		    printk_d("platform device add failed\n");
