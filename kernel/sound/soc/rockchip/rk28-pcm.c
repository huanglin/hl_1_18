...
static const struct snd_pcm_hardware rockchip_pcm_hardware = {
	...
	.period_bytes_max	= 4095*4,///PAGE_SIZE*2,
	.periods_min		= 3,///2,
	.periods_max		= 128,
	.fifo_size		= 16,
};
...
void audio_start_dma(struct rockchip_runtime_data *prtd, int mode)
{
	...
	case DMA_MODE_WRITE:
		...
		if (sg_buf) {
		                #if 0 // 20100701,HSL@RK,test audio broken.
		                unsigned int *i2s_reg = (unsigned int*)(I2S_BASE_ADDR_VA+0x14); // fifo status and int.
		                if( *i2s_reg & ((1<<6)|(1<<2)) ) {  // empty or almost empty.
		                        printk("hsl:irq=%d\n",l_irq );
		                        //dump_stack();
		                }
		                #endif
...
{    
	...
	//printk("%s[0]::total time=%Ld ns\n" , __func__ , ktime_to_ns(now));
	///now = ktime_get();
