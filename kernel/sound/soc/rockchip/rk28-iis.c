	...
	int clk_id, unsigned int freq, int dir)
{
	...
	else if(mclk == 11289600){
		codec_clk_bak = __rockchip_clk_get_uint_clk( SCU_IPID_CODEC );
		rockchip_clk_set_codecdsp(SCU_IPID_CODEC, SCU_CLK_112896);
		__rockchip_scu_set_parent( SCU_IPID_I2S , SCU_IPID_CODEC);
	}
	else{
...
static int rockchip_i2s_probe(struct platform_device *pdev)
{
    ...
    rockchip_i2s_set_sysclk(NULL, 0, 11289600, 0);
