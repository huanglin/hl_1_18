...
//#define WM8988_DBG
#ifdef WM8988_DBG
#define printk_d(fmt, args...)	printk(KERN_INFO "[WM8988] " "%s(%d): " fmt, __FUNCTION__, __LINE__, ##args)
#else
...
struct snd_soc_device *wm8988_socdev = NULL;
 ...
 */
inline int wm8988_read_reg_cache(struct snd_soc_codec *codec, u32 reg)
{
 ...
 */
inline void wm8988_write_reg_cache(struct snd_soc_codec *codec,
	u32 reg, u32 value)
{
	u16 *cache = codec->reg_cache;
...
int wm8988_write(struct snd_soc_codec *codec, u32 reg, u32 value)
{
	u8 data[2];

    printk_d("reg=0x%x value=0x%x\n", reg, value);
...
static const struct snd_soc_dapm_widget wm8988_dapm_widgets[] = 
{
	SND_SOC_DAPM_MIXER("Left Mixer", SND_SOC_NOPM, 0, 0,
		&wm8988_left_mixer_controls[0],	ARRAY_SIZE(wm8988_left_mixer_controls)),
	SND_SOC_DAPM_MIXER("Right Mixer", SND_SOC_NOPM, 0, 0,
		&wm8988_right_mixer_controls[0], ARRAY_SIZE(wm8988_right_mixer_controls)),
...
static inline int get_coeff(int mclk, int rate)
{
	...
	printk_d("could not get coeff for mclk %d @ rate %d\n",mclk, rate);
	return -EINVAL;
}
	...
	struct snd_pcm_hw_params *params)
{
	...
	if (params->flags == HW_PARAMS_FLAG_EQVOL_ON)
	{
		...
		printk_d("HW_PARAMS_FLAG_EQVOL_ON.....\n");
	...
	else if (params->flags == HW_PARAMS_FLAG_EQVOL_OFF)
	{
		...
		printk_d("HW_PARAMS_FLAG_EQVOL_OFF.....\n");
		wm8988_write(codec, WM8988_ADCDAC, r5);
		return 0;
	} 
	
	printk_d("Set new clock: %d\n", params_rate(params));
	/* bit size */
	...
	wm8988_write(codec, WM8988_IFACE, iface);
	if (coeff >= 0) {
		wm8988_write(codec, WM8988_SRATE, srate | (coeff_div[coeff].sr << 1) | coeff_div[coeff].usb);
	}
	wm8988_write(codec,0,  0x010f);
	wm8988_write(codec,1,  0x010f);
	wm8988_write(codec,25, WM_VMID50K|WM_VREF|WM_AINL|WM_ADCL|WM_ADCR|WM_AINR|WM_MICB);   //Left ADC and Left PGA power on,Right ADC and Right PGA power on

	return 0;
}

static int wm8988_mute(struct snd_soc_codec_dai *dai, int mute)
{
...
void wm8988_start_set(struct snd_soc_codec *codec)
{
	...
	wm8988_write(codec, WM8988_LOUTM2, 0x0070);
	wm8988_write(codec, WM8988_ROUTM1, 0x0070);
	...
	wm8988_write(codec, WM8988_LINVOL,	0x011f);
	wm8988_write(codec, WM8988_RINVOL,	0x011f);
	
	wm8988_write(codec, WM8988_ADCTL2, 0x0184);   
	
	/*set Left and Right ADC Digital Volume*/
	wm8988_write(codec, WM8988_LADC, 0x01ec);//0x01c3
	wm8988_write(codec, WM8988_RADC, 0x01ec);
	
	/*set ADC input Mode*/
	wm8988_write(codec, WM8988_ADCIN,  0x0100);
	wm8988_write(codec, WM8988_LADCIN, 0x00f0);//0x00e0
	wm8988_write(codec, WM8988_RADCIN, 0x00f0);//0x00e0
...
static int wm8988_trigger(struct snd_pcm_substream *substream, int trigger)
{
...
static int wm8988_codec_probe(struct i2c_adapter *adap, int addr, int kind)
{
	...
	if (ret < 0) {
		printk("failed to initialise WM8988\n");
		goto err;
	}
...
static int wm8988_probe(struct platform_device *pdev)
{
	...
	struct wm8988_priv *wm8988 = NULL;
	int ret = 0;
	...
	if (wm8988 == NULL) 
	{
	    printk("malloc for struct wm8988_priv failed.\n");
		kfree(codec);
		return -ENOMEM;
	}
