...
#define SPK_IOMUX_PIN_NAME       GPIOE_SPI1_FLASH_SEL_NAME
#define SPK_IOMUX_PIN_DIR        IOMUXA_GPIO1_A3B7
#endif
#define SPK_CTRL_PIN          		SPK_CTL_IOPIN
#define SPK_IOMUX_PIN_NAME       SPK_CTL_IOMUX_PINNAME
#define SPK_IOMUX_PIN_DIR        SPK_CTL_IOMUX_PINDIR
...
int codec_clk_bak = DDR_NORMAL_CLK;

int hdmi_codec_set_clk(int mclk, int rate)
{
	...
	if(mclk == 12000000){
		__rockchip_scu_set_parent( SCU_IPID_I2S , SCU_IPID_12M );
		rockchip_clk_set_codecdsp( SCU_IPID_CODEC , codec_clk_bak);
	}
	else if(mclk == 12288000){
		codec_clk_bak = __rockchip_clk_get_uint_clk( SCU_IPID_CODEC );
		rockchip_clk_set_codecdsp(SCU_IPID_CODEC, SCU_CLK_122880);
		__rockchip_scu_set_parent( SCU_IPID_I2S , SCU_IPID_CODEC);
	}
	else{
...
EXPORT_SYMBOL(hdmi_codec_set_clk);

struct snd_soc_codec_dai rk1000_codec_dai;
static int rk1000_codec_mute(struct snd_soc_codec_dai *dai, int mute);

int hdmi_codec_mute(int mute)
{
	return rk1000_codec_mute(&rk1000_codec_dai, mute);
}
EXPORT_SYMBOL(hdmi_codec_mute);
...
*/
#define RK1000_RATES (SNDRV_PCM_RATE_48000)
